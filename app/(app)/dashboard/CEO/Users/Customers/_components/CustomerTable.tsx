...
export default function CustomerTable({ customers, rolesMap }: Props) {
  return (
    <div className="overflow-hidden rounded-3xl border border-primary-300/20 bg-linear-to-b from-white/4 to-white/2 shadow-xl backdrop-blur-2xl">
      {/* Desktop Table */}
      <div className="hidden lg:block overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead className="border-b border-primary-300/15 bg-white/2 text-xs uppercase tracking-wider text-primary-400">
            <tr>
              <th className="px-8 py-5 text-right font-medium">مشتری</th>
              ...
              <th className="px-8 py-5 text-right font-medium">تاریخ عضویت</th>
            ...
            {customers.map((emp) => (
              ...
              >
                ...
                <td className="px-4 py-6 text-primary-300 flex">
                  <div className="flex items-center gap-2">
                    <Link
                      href={`/dashboard/ceo/users/customers/${emp.id}/edit`}
                      title="ویرایش مشتری"
                  ...
                  <div className="flex items-center gap-2">
                    <Link
                      href={`/dashboard/ceo/users/customers/${emp.id}`}
                      title="اطلاعات مشتری"
                      className="p-2 rounded-lg hover:bg-white/10 transition-colors"
                    >
                      <Eye
      ...
      <div className="lg:hidden flex flex-col gap-4 p-4">
        {customers.map((emp) => (
          <div
            key={emp.id}
            className="flex flex-col gap-2 p-4 rounded-2xl bg-white/4 border border-primary-300/10 shadow backdrop-blur-md"
          >
